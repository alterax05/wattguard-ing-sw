...
describe("evaluateEfficiencyAlerts", () => {
  ...
  test("does nothing when threshold disabled", async () => {
    const building = await createBuilding({
      efficiencyThresholds: { enabled: false, minCop: 10 },
    });
    const bid = building._id.toString();
    ...
    const count = await Alert.countDocuments({ type: EFFICIENCY_ALERT_TYPE });
    expect(count).toBe(0);
  });

  test("does nothing when averageCop is null (insufficient data)", async () => {
    const building = await createBuilding({
      efficiencyThresholds: { enabled: true, minCop: 10 },
    });
    const bid = building._id.toString();
    ...
    // Only internal temp readings, no energy meter → no heating phase → COP null
    const now = new Date();
    const t0 = new Date(now.getTime() - 10 * 60_000);
    await SensorReading.insertMany([
      mkReading(t0, 22.0, "°C", "internal_temp", iSensor._id.toString(), bid),
      mkReading(new Date(t0.getTime() + 60_000), 21.8, "°C", "internal_temp", iSensor._id.toString(), bid),
    ]);
  ...
  test("does nothing for district heating buildings", async () => {
    const building = await createBuilding({
    ...
    // COP is forced null for district heating → silent
    const count = await Alert.countDocuments({ type: EFFICIENCY_ALERT_TYPE });
    expect(count).toBe(0);
  });
});
